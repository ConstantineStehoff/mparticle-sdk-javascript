...
function send(event) {
    ...
    if (!tryNativeSdk(NativeSdkPaths.LogEvent, JSON.stringify(event))) {
        ...
        if (xhr) {
            try {
                xhr.open('post', utilities.createServiceUrl(serviceScheme, secureServiceUrl, serviceUrl, devToken) + '/Events');
...
function sendForwardingStats(forwarder, event) {
    ...
    if (forwarder && forwarder.isVisible) {
        ...
        if (xhr) {
            try {
                xhr.open('post', utilities.createServiceUrl(serviceScheme, secureServiceUrl, serviceUrl, devToken) + '/Forwarding');
    ...
    if (forwarders) {
        for (var i = 0; i < forwarders.length; i++) {
            if (forwarders[i].setUserAttribute &&
                ...
                !utilities.inArray(forwarders[i].userAttributeFilters, generateHash(key))) {
                    ...
                    for (var i = 0; i < forwarders.length; i++) {
                        if (forwarders[i].setUserIdentity &&
                            (!forwarders[i].userIdentityFilters ||
                                !utilities.inArray(forwarders[i].userIdentityFilters, type))) {
